		...
		if ($_SESSION['rand'] == $_POST['randcheck']) {
			...
			if (!isset($emptyNameError) && $countName < 51 && !isset($emptyEmailError) && $countEmail < 81 && !isset($invalidEmailError) && !isset($emptyMessageError) && $countMessage < ($maxMessageChar + 1) && isset($emptySubjectError) && !empty($_POST['g-recaptcha-response'])) {
				...
				if ($valid["success"] == true) {
					...
					if ($mail) {
						echo '<div class="info success"><i class="fa-solid fa-envelope"></i>Thank you for contacting me! Your message has been sent. I&lsquo;ll respond your message within 2x24 hours.</div>';
						$emailSent = true;
					} else {
...
?>
	<div class="info danger"><i class="fa-solid fa-triangle-exclamation"></i> Failed to create contact form.</div>
	<div class="info danger"><i class="fa-solid fa-triangle-exclamation"></i> Please set your reCAPTCHA site key, secret key and add your email address in Theme Settings.</div>
<?php
}
